const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'SLAI 賬號',
    langGeniusAccountTip: '您的 SLAI 賬號和相關的使用者資料。',
    editName: '編輯名字',
  ...
  members: {
    ...
    invitationSentTip: '邀請已傳送，對方登入 SLAI 後即可訪問你的團隊資料。',
    invitationLink: '邀請連結',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '體驗',
      exhausted: '超出限額',
      desc: '託管 OpenAI 由 SLAI 提供的託管 OpenAI 服務，你可以使用 GPT-3.5 等模型，在體驗額度消耗完畢前你需要設定其它模型供應商。',
      callTimes: '呼叫次數',
  ...
  apiBasedExtension: {
    title: 'API 擴充套件提供了一個集中式的 API 管理，在此統一新增 API 配置後，方便在 SLAI 上的各類應用中直接使用。',
    link: '瞭解如何開發您自己的 API 擴充套件。',
    linkUrl: 'https://docs.shuling.ai/v/zh-hans/guides/extension/api-based-extension',
    add: '新增 API 擴充套件',
    selector: {
      title: 'API 擴充套件',
  ...
  about: {
    changeLog: '更新日誌',
    updateNow: '現在更新',
    nowAvailable: 'SLAI {{version}} 現已可用。',
    latestAvailable: 'SLAI {{version}} 已是最新版本。',
  },
  appMenus: {
    overview: '監控',
    promptEng: '編排',
  ...
  imageUploader: {
    ...
    uploadFromComputerTypeLimit: '圖片類型為 {{type}}',
  ...
  fileUploader: {
    ...
    uploadFromComputerTypeLimit: '文件類型為 {{type}}',
    pasteFileLink: '貼上文件連結',
    pasteFileLinkInputPlaceholder: '將文件連結貼上到此處',
    pasteFileLinkInvalid: '文件連結無效',
    fileUpload: '文件上傳',
  },
  tag: {
    placeholder: '全部標籤',
