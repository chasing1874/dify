const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Tài khoản SLAI',
    langGeniusAccountTip: 'Tài khoản SLAI của bạn và dữ liệu người dùng liên quan.',
  ...
  members: {
    ...
    invitationSentTip: 'Lời mời đã được gửi, và họ có thể đăng nhập vào SLAI để truy cập vào dữ liệu nhóm của bạn.',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: 'DÙNG THỬ',
      exhausted: 'HẾT QUOTA',
      desc: 'Dịch vụ lưu trữ OpenAI được cung cấp bởi SLAI cho phép bạn sử dụng các mô hình như GPT-3.5. Trước khi hết lượng truy vấn dùng thử, bạn cần thiết lập các nhà cung cấp mô hình khác.',
  ...
  apiBasedExtension: {
    title: 'Các tiện ích API cung cấp quản lý API tập trung, giúp cấu hình dễ dàng sử dụng trên các ứng dụng của SLAI.',
  ...
  about: {
    ...
    nowAvailable: 'SLAI {{version}} hiện đã có sẵn.',
    latestAvailable: 'SLAI {{version}} là phiên bản mới nhất hiện có.',
  },
  appMenus: {
    overview: 'Giám sát',
  ...
  imageUploader: {
    ...
    uploadFromComputerTypeLimit: 'Loại ảnh phải là {{type}}',
  ...
  fileUploader: {
    ...
    uploadFromComputerUploadError: 'Tải tệp lên thất bại, vui lòng tải lại.',
    uploadFromComputerLimit: 'Tệp tải lên không được vượt quá {{size}} MB',
    uploadFromComputerTypeLimit: 'Loại tệp phải là {{type}}',
    pasteFileLink: 'Dán liên kết tệp',
    pasteFileLinkInputPlaceholder: 'Dán liên kết tệp vào đây',
