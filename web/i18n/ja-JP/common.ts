const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'SLAIアカウント',
    langGeniusAccountTip: 'SLAIアカウントと関連するユーザーデータ。',
    editName: '名前を編集',
  ...
  members: {
    ...
    invitationSentTip: '招待が送信され、彼らはSLAIにサインインしてあなた様のチームデータにアクセスできます。',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'SLAIが提供するOpenAIホスティングサービスを使用すると、GPT-3.5などのモデルを使用できます。トライアルクォータが使い果たされる前に、他のモデルプロバイダを設定する必要があります。',
      callTimes: '通話回数',
  ...
  about: {
    changeLog: '変更ログ',
    updateNow: '今すぐ更新',
    nowAvailable: 'SLAI {{version}} が利用可能です。',
    latestAvailable: 'SLAI {{version}} が最新バージョンです。',
  },
  appMenus: {
    overview: '監視',
    promptEng: 'オーケストレート',
  ...
  imageUploader: {
    ...
    uploadFromComputerTypeLimit: '画像タイプは {{type}} です',
  ...
  fileUploader: {
    uploadFromComputer: 'コンピューターからアップロード',
    uploadFromComputerReadError: 'ファイルの読み込みに失敗しました。もう一度選択してください。',
    uploadFromComputerUploadError: 'ファイルのアップロードに失敗しました。もう一度アップロードしてください。',
    uploadFromComputerLimit: 'アップロードできるファイルは {{size}} MB を超えることはできません',
    uploadFromComputerTypeLimit: 'ファイルタイプは {{type}} です',
    pasteFileLink: 'ファイルリンクを貼り付ける',
